...
export default function BaseInputTemplate(props) {
    ...
    const inputProps = getInputProps(schema, getDefaultInputType(schema, type), options);
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(TextField, Object.assign({ "data-sign": name, id: id, placeholder: placeholder, label: labelValue(label || undefined, hideLabel, false), fullWidth: true, 
            // eslint-disable-next-line jsx-a11y/no-autofocus
            autoFocus: autofocus, required: required, disabled: disabled || readonly, inputProps: Object.assign({ step,
                min,
                max }, (schema.examples ? { list: examplesId(id) } : undefined)) }, rest, { value: value || value === 0 ? value : '', error: Boolean(errorMessage), helperText: errorMessage, onChange: onChangeOverride || handleChange, onBlur: handleBlur, onFocus: handleFocus }, textFieldProps, { size: "large", type: getTextFieldType(inputType), "aria-describedby": ariaDescribedByIds(id, !!schema.examples) })),
        Array.isArray(schema.examples) ? (React.createElement("datalist", { id: examplesId(id) }, schema.examples
