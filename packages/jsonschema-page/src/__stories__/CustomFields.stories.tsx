...
const meta: Meta<typeof JSONSchemaPage> = {
  title: 'JSONSchemaPage/Custom Fields',
  component: JSONSchemaPage,
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component: 'Examples of custom field types available in JSONSchemaPage',
      },
    },
  },
  tags: ['autodocs'],
...
// Interactive Form with Multiple Custom Fields
export const InteractiveForm: Story = {
  args: {
    schema: {
      title: 'Interactive Dashboard Form',
      type: 'object',
      properties: {
        userInfo: {
          type: 'object',
        ...
        searchQuery: {
          type: 'string',
          title: 'Search',
          description: 'Search through available options',
        },
        statusAlert: {
          type: 'null',
          title: 'System Status',
    ...
    uiSchema: {
      ...
      statusAlert: {
        ...
        'ui:options': {
          severity: 'info',
          title: 'System Status',
          message: 'All systems are operational. Last updated: ' + new Date().toLocaleTimeString(),
          closable: false,
        },
      },
      quickActions: {
        ...
        'ui:options': {
          ...
          onClick: (formData: any) => {
            const actions = ['Export Data', 'Import Settings', 'Generate Report', 'Send Notification'];
      ...
      resetForm: {
        ...
        'ui:options': {
          ...
          color: 'error',
          text: 'Reset All',
          onClick: (formData: any, helpers: any) => {
            if (window.confirm('Are you sure you want to reset all data?')) {
              if (helpers && helpers.resetForm) {
                helpers.resetForm();
              }
  ...
  render: (args) => {
    ...
    const enhancedArgs = {
      ...
      uiSchema: {
        ...args.uiSchema,
        searchQuery: {
          ...(args.uiSchema as any).searchQuery,
          'ui:options': {
            ...(args.uiSchema as any).searchQuery['ui:options'],
            onSearch: (query: string) => {
              setSearchHistory(prev => ({
                ...prev,
                global: [...(prev.global || []), query].slice(-3),
              }));
              console.log(`Global search for: ${query}, found ${Math.floor(Math.random() * 50) + 1} results`);
            },
          },
        },
        resetForm: {
          ...(args.uiSchema as any).resetForm,
          'ui:options': {
            ...(args.uiSchema as any).resetForm['ui:options'],
            onClick: (data: any, helpers: any) => {
              if (window.confirm('Are you sure you want to reset all data?')) {
    ...
    return (
      <StoryLayout 
        ...
        resultComponent={
          <>
            <h4 style={{ margin: '0 0 10px 0', color: '#555' }}>📊 Activity Log</h4>
            <div style={{ fontSize: '12px', marginBottom: '15px' }}>
              <p>Search History:</p>
              <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
                {Object.entries(searchHistory).map(([type, queries]) => (
                  <li key={type}>{type}: {queries?.join(', ') || 'None'}</li>
                ))}
              </ul>
            </div>
            <h4 style={{ margin: '15px 0 10px 0', color: '#555' }}>📝 Current Data</h4>
            <pre style={{ fontSize: '11px', overflow: 'auto', maxHeight: '200px', margin: 0, background: 'white', padding: '10px', borderRadius: '4px', border: '1px solid #ddd' }}>
              {JSON.stringify(formData, null, 2)}
            </pre>
          </>
        ...
        resultComponent={
          <>
            ...
            <div style={{ fontSize: '12px', marginBottom: '15px' }}>
              ...
              <p>Author Clicks: {authorClicks}</p>
              <p>Project Status: {
