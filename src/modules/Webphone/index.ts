...
import { isRing, sortByLastActiveTimeDesc } from '@ringcentral-integration/commons/modules/Webphone/webphoneHelper';
import { MultipleTabsTransport } from '../../lib/MultipleTabsTransport';
import { normalizeSession, isDroppingVoicemail } from './helper';
...
export class Webphone extends WebphoneBase {
  ...
  async _stopSessionAudio(webphoneSession) {
    await this._deps.noiseReduction.reset(webphoneSession.id);
    ...
    const outputElement = this._remoteVideo;
  ...
  async dropVoicemailMessage(sessionId, messageId) {
    ...
    if (
      ...
      webphoneSession.localHold ||
      !webphoneSession.sessionDescriptionHandler ||
      !webphoneSession.sessionDescriptionHandler.peerConnection
    ) {
      return false;
    }
    try {
      ...
      webphoneSession.localHold = true; // prevent the call be held when user start another call
      this._updateSessions();
      ...
      this._deps.voicemailDrop.dropVoicemailMessage({
        ...
        audioContext,
        endCall: () => {
          return this.hangup(webphoneSession.id);
        },
        updateStatus: (status) => {
  ...
  @action
  override _setStateOnCallEnd(session: NormalizedSession) {
    if (this.activeSessionId === session.id) {
      // filter out ringing and dropping voicemail sessions
      const activeSessions = this.sessions.filter((x) => (!isRing(x) && !isDroppingVoicemail(x.voicemailDropStatus)));
